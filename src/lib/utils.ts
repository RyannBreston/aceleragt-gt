...
export const incrementAttendance = onCall(corsOptions, async (request) => {
  if (request.auth?.token.role !== "seller") {
    throw new HttpsError("permission-denied", "Apenas vendedores podem registar atendimentos.");
  }
  ...
  try {
    await db.runTransaction(async (transaction) => {
      ...
      const isSameDay = lastUpdate ? 
        ...
        lastUpdate.getFullYear() === now.getFullYear() : false;

      const newCount = isSameDay ? (sellerData?.dailyAttendanceCount || 0) + 1 : 1;
    ...
    return {result: "Atendimento registado com sucesso."};
  } catch (error) {
    throw new HttpsError("internal", "Ocorreu um erro ao registar o atendimento.");
  }
});
