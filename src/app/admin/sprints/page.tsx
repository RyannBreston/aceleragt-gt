...
import { Loader2, PlusCircle, Save, Zap, Users } from "lucide-react";
...
const SprintFormModal = ({ isOpen, setIsOpen, onSave, sellers }: { isOpen: boolean; setIsOpen: (open: boolean) => void; onSave: (sprintData: any) => Promise<void>; sellers: Seller[] }) => {
    const [title, setTitle] = useState('');
    const [sprintTiers, setSprintTiers] = useState<SprintTier[]>([
        { goal: 200, prizePoints: 50 }, { goal: 400, prizePoints: 100 },
        { goal: 600, prizePoints: 150 }, { goal: 800, prizePoints: 250 },
    ]);
    const [participantIds, setParticipantIds] = useState<string[]>([]);
    const [isSubmitting, setIsSubmitting] = useState(false);

    const handleTierChange = (index: number, field: 'goal' | 'prizePoints', value: number) => {
    ...
    const handleSave = async () => {
        ...
        const sprintData = { title, participantIds, sprintTiers };
        
        console.log("A enviar para a Cloud Function:", sprintData); // Depuração
        ...
        <Dialog open={isOpen} onOpenChange={setIsOpen}>
            <DialogContent className="max-w-2xl">
                <DialogHeader>
                    ...
                    <DialogDescription>Defina múltiplos níveis de objetivos de vendas para a sua equipa.</DialogDescription>
                </DialogHeader>
                <div className="space-y-4 py-4 max-h-[70vh] overflow-y-auto pr-4">
                    <div className="space-y-2"><Label>Título da Corridinha</Label><Input value={title} onChange={(e) => setTitle(e.target.value)} placeholder="Ex: Sprint de Vendas Relâmpago" /></div>
                    <div className="space-y-4 rounded-md border p-4">
                        <Label className="font-semibold">Níveis de Metas e Prémios</Label>
                        {sprintTiers.map((tier, index) => (
                            <div key={index} className="flex items-center gap-4">
                                <Label className="w-16 text-sm text-muted-foreground">Nível {index + 1}</Label>
                                <Input type="number" placeholder="Meta (R$)" value={tier.goal} onChange={(e) => handleTierChange(index, 'goal', Number(e.target.value))} />
                                <Input type="number" placeholder="Prémio (Pts)" value={tier.prizePoints} onChange={(e) => handleTierChange(index, 'prizePoints', Number(e.target.value))} />
                            </div>
                        ))}
                    </div>
                    <div>
...
export default function AdminSprintsPage() {
    ...
    const handleSaveSprint = async (sprintData: Omit<DailySprint, 'id' | 'createdAt' | 'isActive'>) => {
        try {
            const createSprintFunction = httpsCallable(functions, 'createDailySprint');
            const response = await createSprintFunction(sprintData);
            console.log("Resposta do Backend:", response.data); // Depuração
            toast({ title: 'Corridinha criada com sucesso!' });
        } catch (error: any) {
            console.error("Erro ao chamar a Cloud Function:", error);
            toast({ variant: 'destructive', title: 'Erro ao criar corridinha', description: error.message });
        }
    };
    ...
    return (
        <div className="space-y-8">
            ...
            <Card>
                <CardHeader><CardTitle>Histórico de Corridinhas</CardTitle><CardDescription>Lista de todas as corridinhas criadas, da mais recente para a mais antiga.</CardDescription></CardHeader>
                <CardContent>
                            ...
                            <TableBody>
                                ...
                                sprints.length > 0 ? sprints.map(sprint => (
                                    <TableRow key={sprint.id} className={sprint.isActive ? 'bg-primary/5' : ''}>
                                        ...
                                        <TableCell>Até {Math.max(...sprint.sprintTiers.map(t => t.goal)).toLocaleString('pt-BR', {style: 'currency', currency: 'BRL'})}</TableCell>
                                        <TableCell>{sprint.participantIds.length} / {sellers.length}</TableCell>
                                        <TableCell>{sprint.createdAt ? format(sprint.createdAt.seconds * 1000, 'dd/MM/yyyy', { locale: ptBR }) : 'N/A'}</TableCell>
                                        <TableCell><span className={sprint.isActive ? "font-semibold text-green-500" : ""}>{sprint.isActive ? "Ativa" : "Finalizada"}</span></TableCell>
                                    </TableRow>
                                )) :
