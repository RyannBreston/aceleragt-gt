...
import { 
    Loader2, BookCopy, Trash2, GraduationCap, FileText, Search, Users, 
    ...
    return (
        <DialogContent className="max-w-4xl bg-transparent border-none shadow-none p-0">
            <DialogHeader className="sr-only">
                <DialogTitle>Certificado: {course.title}</DialogTitle>
            </DialogHeader>

            <div className="absolute top-4 left-4 z-20 w-64">
                <Label className="text-white font-semibold text-shadow">
                    Pré-visualizar para:
                </Label>
            ...
            <Certificate
                ...
                achievements={["Competência Chave"]}
            />
        </DialogContent>
    );
};
...
export default function AcademiaPage() {
    ...
    const courseCompletions = useMemo(() => {
        ...
        courses.forEach(course => {
            completions.set(
                course.id!,
                sellers.filter(s => s.completedCourseIds?.includes(course.id!)).length
            );
        });
        return completions;
    }, [courses, sellers]);
            ...
            <div className="space-y-4">
                <div className="flex justify-between items-center">
                    <h3 className="text-xl font-semibold">
                        Cursos Criados ({filteredCourses.length})
                    </h3>
                                ...
                                <Card key={course.id} className="flex flex-col">
                                    <CardHeader>
                                        ...
                                        <CardDescription className="flex items-center gap-1.5 text-xs">
                                            ...
                                            <span>
                                                {courseCompletions.get(course.id!) || 0} / {sellers.length} concluíram
                                            </span>
                                        </CardDescription>
                                    </CardHeader>
                                        ...
                                        <Dialog>
                                            <DialogTrigger asChild>
                                                <Button variant="outline" size="sm" className="flex-1">
                                                    <FileText className="mr-2 size-4" /> Certificado
                                                </Button>
                                            </DialogTrigger>
                                        ...
                                        <AlertDialog>
                                            ...
                                            <AlertDialogContent>
                                                <AlertDialogHeader>
                                                    ...
                                                    <AlertDialogDescription>
                                                        Esta ação não pode ser desfeita e irá remover o curso permanentemente.
                                                    </AlertDialogDescription>
                                                </AlertDialogHeader>
                                                <AlertDialogFooter>
